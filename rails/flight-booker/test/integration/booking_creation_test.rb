  ...
  def test_create_valid_booking
    ...
    assert_difference 'Booking.count', 1 do
      assert_difference 'Passenger.count', 3 do
        post_via_redirect '/bookings', booking: {flight_id: @flight.id, passenger_count: 3, passenger: [{name: "A User", email: "a_user@example.com"}, {name: "C. Douglass", email: "cdouglass@example.com"}, {name: "John Smith", email: "jsmith@example.com"}]}
      end
    end
  ...
  def test_create_booking_with_invalid_passengers
    ...
    assert_no_difference 'Booking.count' do
      assert_no_difference 'Passenger.count' do
        post_via_redirect '/bookings', {booking: {flight_id: @flight.id, passenger_count: 3, passenger: [{name: "", email: "a_user@example.com"}, {name: "C. Douglass", email: "cdouglass@example.com"}, {name: "John Smith", email: "jsmith@example.com"}]}}, {'HTTP_REFERER' => "/bookings/new?passenger_count=1&flight_id=#{@flight.id}&commit=Book+flight!"}
      end
    end
    refute flash.empty?
